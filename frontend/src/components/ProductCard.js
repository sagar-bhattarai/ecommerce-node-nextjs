    ...
    <div className="relative w-full max-w-[18rem] bg-neutral-primary-soft p-6 border border-default rounded-sm shadow-xs">
      ...
      <div>
        ...
        <div className="flex flex-col">
          <Link key={product._id} href={`/products/${product._id}`} className="text-xl text-heading font-semibold tracking-tight">{product.productName}</Link>
          <span className="text-xs font-medium py-0.5 ">category name</span>
        </div>
        <div className="flex items-center justify-between mt-6">
          <span className="text-3xl font-extrabold text-heading">RS.{product?.productPrice ?? "N/A"} </span>
